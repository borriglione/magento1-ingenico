Event.observe(
    window, 'load', function () {
        ...
        payment.prepareOpsForm = function (hash) {
            ...
            if (payment.currentMethodObject.transmitPaymentMethod === true) {
        ...
        payment.fillOpsLoader = function (token, message) {
            if (token) {
                var messageString = Translator.translate(token);
                if (typeof message !== 'undefined' && typeof message !== 'object') {
                    messageString = message + ' ' + messageString;
                }
                payment.currentMethodObject.loader.innerHTML = messageString;
        ...
        payment.onOpsIframeLoad = function () {
            ...
            if (payment.currentMethodObject && payment.currentMethodObject.tokenizationFrame.src != 'about:blank' && !payment.opsAliasSuccess) {
                payment.currentMethodObject.loader.style.display = 'none';
